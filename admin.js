// functions
{
    ...
    function checkBlock() {
        ...
        customerStatus.forEach((status) => {
            const emailLock = status.parentElement.parentElement.querySelector(".customer__userEmail").innerText;
    ...
    function editProduct(productElement) {
        ...
        for (let i = 0; i < products.length; i++) {
            if (products[i].ID == id) {
                ...
                `;
                    divProduct.innerHTML = productContent;
                    document.querySelector(".product__edit").style.removeProperty("display");
                });
                break;
            }
        }
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // *IF USER NOT LOGIN (userLogin not exists) => PROMPT USER TO LOGIN*
